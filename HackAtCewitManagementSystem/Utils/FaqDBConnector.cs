...
namespace HackAtCewitManagementSystem.Utils
{
    public static class FaqDBConnector
    {
        ...
        private static bool InsertOrUpdate(string dataSource, Faq faq) {
            ...
            {
                conn.Open();
                SqliteCommand insertSQL = new SqliteCommand("INSERT OR REPLACE INTO Faq(Id, Question, Answer) VALUES ((SELECT Id FROM Faq WHERE Id = @id), @question, @answer)", conn);
                insertSQL.Parameters.AddWithValue("@id", faq.Id);
                insertSQL.Parameters.AddWithValue("@question", faq.Question);
                insertSQL.Parameters.AddWithValue("@answer", faq.Answer);
