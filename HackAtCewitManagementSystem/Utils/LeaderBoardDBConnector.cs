    ...
    public static class LeaderBoardDBConnector
    {
        ...
        private static bool InsertOrUpdate(string dataSource, LeaderBoard leaderBoard)
        {
            using (SqliteConnection conn = new SqliteConnection(dataSource))
            {
                ...
                conn.Open();
                SqliteCommand insertSQL = new SqliteCommand("INSERT OR REPLACE INTO LeaderBoard(Username, Score, LastUpdated) VALUES (@username, @score, datetime('now', 'localtime'))", conn);
                insertSQL.Parameters.AddWithValue("@username", leaderBoard.Username);
                insertSQL.Parameters.AddWithValue("@score", leaderBoard.Score);
