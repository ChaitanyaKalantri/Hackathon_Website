...
namespace HackAtCewitManagementSystem.Utils
{
    public static class ResourceDBConnector
    {
        ...
        private static bool InsertOrUpdate(string dataSource, Resource resource)
        {
            ...
            {
                conn.Open();
                SqliteCommand insertSQL = new SqliteCommand("INSERT OR REPLACE INTO Resource(Id, Title, Link, Description, ProviderName) VALUES ((SELECT Id FROM Resource WHERE Id = @id), @title, @link, @description, @providerName)", conn);
                insertSQL.Parameters.AddWithValue("@id", resource.Id);
                insertSQL.Parameters.AddWithValue("@title", resource.Title);
                insertSQL.Parameters.AddWithValue("@link", resource.Link);
                insertSQL.Parameters.AddWithValue("@description", resource.Description);
                insertSQL.Parameters.AddWithValue("@providerName", resource.ProviderName);
