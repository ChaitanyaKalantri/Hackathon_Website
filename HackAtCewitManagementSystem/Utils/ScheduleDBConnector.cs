...
namespace HackAtCewitManagementSystem.Utils
{
    public static class ScheduleDBConnector
    {
        ...
        private static bool InsertOrUpdate(string dataSource, Schedule schedule)
        {
            ...
            {
                conn.Open();
                SqliteCommand insertSQL = new SqliteCommand("INSERT OR REPLACE INTO Schedule(Id, StartTime, EndTime, EventDescription, Room, EventTitle, Presenter) VALUES ((SELECT Id FROM Schedule WHERE Id = @id), @startTime, @endTime, @eventDescription, @room, @eventTitle, @presenter)", conn);
                insertSQL.Parameters.AddWithValue("@id", schedule.Id);
                insertSQL.Parameters.AddWithValue("@startTime", schedule.StartTime);
                insertSQL.Parameters.AddWithValue("@endTime", schedule.EndTime);
                insertSQL.Parameters.AddWithValue("@eventDescription", schedule.EventDescription);
                insertSQL.Parameters.AddWithValue("@room", schedule.Room);
                insertSQL.Parameters.AddWithValue("@eventTitle", schedule.EventTitle);
                insertSQL.Parameters.AddWithValue("@presenter", schedule.Presenter);
