    ...
    public static class VideoDBConnector
    {
        ...
        private static bool InsertOrUpdate(string dataSource, Video video)
        {
            using (SqliteConnection conn = new SqliteConnection(dataSource))
            {
                ...
                SqliteCommand insertSQL = new SqliteCommand("INSERT OR REPLACE INTO Video(Id, Title, Url) VALUES ((SELECT Id FROM Video WHERE Id = @id), @title, @url)", conn);
                insertSQL.Parameters.AddWithValue("@id", video.Id);
                insertSQL.Parameters.AddWithValue("@title", video.Title);
                insertSQL.Parameters.AddWithValue("@url", video.Url);
